<template>
  <div class="order_line" style="overflow: inherit">
    ...
    <el-upload :action="action" :before-upload="beforeUpload" :data="dataObj" :file-list="fileList" :multiple="false" :on-preview="handlePreview" :on-remove="handleRemove" :on-success="handleUploadSuccess" :show-file-list="showFileList" class="mydefinewidth" list-type="picture" style="z-index:199" :on-progress="handleOnProgress">
  ...
  methods: {
    ...
    beforeUpload(file) {
      ...
      if (this.fileType == "img") {
        ...
        if (!isLtM) {
          this.$message.error("上传媒体文件大小不能超过 500MB!");
        }
        return isJPG && isLtM;
      } else {
    ...
    handleOnProgress(event, file, fileList) {
      console.log(event)
      this.$emit("handleOnProgress", event.percent);
    }
  },
