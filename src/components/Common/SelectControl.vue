  ...
  <div class="order_line">
    ...
    <span class="controlspans">
      <el-select :disabled="disabled" :placeholder="placeholder" :size="size" @change="valuechange" class="mydefinewidth" v-model="currentvalue">
        <el-option v-if="!multi" :key="item.value" :label="item.text" :value="item.value" v-for="item in itemlists"></el-option>
        <el-option v-if="multi" :key="item.value" :label="item.text" :value="item.value" v-for="item in otherLists"></el-option>
      </el-select>
    </span>
  </div>
</template>
    ...
    loaditems: function () {
      if (!u.isnullorwhitespace(this.querypk)) {
        ...
        if (this.pk === "YN") {
          ...
          // 获取所选部门的服务专员和健康专员
          if (this.refDeptId){
            member.getAllEmployee(this.refDeptId).then(res => {
              console.log("=======>",res)

              let healthers = res.healthers;
