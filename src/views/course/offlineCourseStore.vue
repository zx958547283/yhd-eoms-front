...
export default {
  ...
  methods: {
    ...
    showEdit(row) {
      let that = this;
      u.openeditmodel(this, course.getOfflineCourseStore, row).then(res => {
        console.log(res)
        that.timeValue = [res.start_time, res.end_time]
      });;
    },
    doDelete() {
      u.deleteoperate(this, course.deleteOfflineCourseStore);
    },
    doExport() {
    ...
    doSave() {
            u.doSave(this, course.saveOfflineCourseStore);
    },
