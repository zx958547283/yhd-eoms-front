  ...
  <div class="pagecontent">
    ...
    <cwindow :title="wintitle" v-model="winvisible">
      <div class="page-controlstyle">
                                                              <cinput title="线下课程id" v-model="mm.offline_course_id"></cinput>
                                                                              <cinput title="门店id" v-model="mm.store_id"></cinput>
                                                                              <cinput title="会员id" v-model="mm.member_id"></cinput>
                                                                              <cinput title="报名对应订单id" v-model="mm.order_id"></cinput>
                                                                                                              <cdate title="报名时间" v-model="mm.enroll_time"></cdate>
                                                                                                                              </div>
...
export default {
  ...
  methods: {
    ...
    showEdit(row) {
      let that = this;
      u.openeditmodel(this, course.getMemberEnrollOffline, row).then(res => {
        console.log(res)
        that.timeValue = [res.start_time, res.end_time]
      });;
    },
    doDelete() {
      u.deleteoperate(this, course.deleteMemberEnrollOffline);
    },
    doExport() {
    ...
    doSave() {
            u.doSave(this, course.saveMemberEnrollOffline);
    },
