  ...
  <div class="pagecontent">
    ...
    <cwindow :title="wintitle" v-model="winvisible">
      <div class="page-controlstyle">
                                                              <cinput :required="true" title="线下课程报名表id" v-model="mm.enroll_id"></cinput>
                                                                              <cdate :required="true" title="打卡时间" v-model="mm.clock_in_time"></cdate>
                                                                              <cinput title="打卡地点" v-model="mm.clock_location"></cinput>
                                                                                                                              </div>
...
export default {
  ...
  methods: {
    ...
    showEdit(row) {
      let that = this;
      u.openeditmodel(this, course.getMemberClockIn, row).then(res => {
        console.log(res)
        that.timeValue = [res.start_time, res.end_time]
      });;
    },
    doDelete() {
      u.deleteoperate(this, course.deleteMemberClockIn);
    },
    doExport() {
    ...
    doSave() {
            u.doSave(this, course.saveMemberClockIn);
    },
