<template>
  <div class="app-container">
    <div class="table-layout">
      ...
      <el-row>
        <el-col :span="4" class="table-cell" style="font-size: 13px">{{coupon.startTime|formatDate}}至{{coupon.endTime|formatDate}}</el-col>
      ...
      <div style="margin-top: 15px">
        <el-form :inline="true" :model="listQuery" label-width="140px" size="small">
          <el-form-item label="使用状态：">
            <el-select class="input-width" clearable placeholder="全部" v-model="listQuery.useStatus">
              <el-option :key="item.value" :label="item.label" :value="item.value" v-for="item in useTypeOptions"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="订单编号：">
            <el-input class="input-width" placeholder="订单编号" v-model="listQuery.orderSn"></el-input>
          </el-form-item>
        </el-form>
      </div>
    </el-card>
    <div class="table-container">
      <el-table :data="list" border ref="couponHistoryTable" style="width: 100%;" v-loading="listLoading">
...
export default {
  ...
  created() {
    getCoupon(utility.getpageparmas(this)[0]).then(response => {
      this.coupon = response.data;
    });
    this.listQuery.couponId = utility.getpageparmas(this)[0];
    this.getList();
  },
  filters: {
    formatType(type) {
