<template>
  <div class="app-container">
      ...
      <div style="margin-top: 15px">
        <el-form :inline="true" :model="listQuery" label-width="140px" size="small">
          ...
          <el-form-item label="推荐状态：">
            <el-select class="input-width" clearable placeholder="全部" v-model="listQuery.recommendStatus">
              <el-option :key="item.value" :label="item.label" :value="item.value" v-for="item in recommendOptions"></el-option>
            </el-select>
          </el-form-item>
        </el-form>
      </div>
    ...
    <div class="table-container">
      <el-table :data="list" @selection-change="handleSelectionChange" border ref="newProductTable" style="width: 100%;" v-loading="listLoading">
        ...
        <el-table-column align="center" label="是否推荐" width="200">
          <template slot-scope="scope">
            <el-switch :active-value="1" :inactive-value="0" @change="handleRecommendStatusStatusChange(scope.$index, scope.row)" v-model="scope.row.recommend_status"></el-switch>
          </template>
        </el-table-column>
  ...
  methods: {
    ...
    handleSelectDialogConfirm() {
      ...
      this.$confirm('使用要进行添加操作?', '提示', {
        ...
        sms.addHomeHotProductRecommendBatch({ selectProducts: selectProducts }).then(response => {
    ...
    handleUpdateSort() {
      this.$confirm('是否要修改排序?', '提示', {
        ...
        sms.updateHotProductRecommendSort(this.sortDialogData).then(response => {
    ...
    getList() {
      ...
      sms.getHomeHotProductList(this.listQuery).then(res => {
    ...
    updateRecommendStatusStatus(ids, status, exc_keys) {
      this.$confirm('是否要修改推荐状态?', '提示', {
        ...
        sms.updateHotProductRecommendStatus({
          ids: ids,
          exc_keys: exc_keys,
          status: status
        }).then(response => {
    ...
    deleteProduct(ids) {
      this.$confirm('是否要删除该推荐?', '提示', {
        ...
        sms.deleteHomeRecommendProduct({ pk: ids.join(",") }).then(response => {
          this.getList();
          this.$message({
            type: 'success',
            message: '删除成功!'
