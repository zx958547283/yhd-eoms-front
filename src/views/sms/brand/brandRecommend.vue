<template>
  <div class="app-container">
      ...
      <div style="margin-top: 15px">
        <el-form :inline="true" :model="listQuery" label-width="140px" size="small">
          <el-form-item label="品牌名称：">
            <el-input class="input-width" placeholder="品牌名称" v-model="listQuery.brandName"></el-input>
          </el-form-item>
    ...
    <el-card class="operate-container" shadow="never">
      ...
      <span>数据列表</span>
      <el-button @click="handleSelectBrand()" class="btn-add" size="mini">选择品牌</el-button>
    </el-card>
    ...
    <el-dialog :visible.sync="selectDialogVisible" title="选择品牌" width="40%">
      ...
      <el-table :data="dialogData.list" @selection-change="handleDialogSelectionChange" border>
        ...
        <el-table-column align="center" label="相关" width="220">
          <template slot-scope="scope">
            商品：
            <span class="color-main">{{scope.row.product_count}}</span>
            评价：
            <span class="color-main">{{scope.row.product_comment_count}}</span>
          </template>
        </el-table-column>
      </el-table>
      <div class="pagination-container">
  ...
  methods: {
    ...
    handleSelectDialogConfirm() {
      ...
      this.$confirm('使用要进行添加操作?', '提示', {
        ...
        sms.addHomeBrandRecommendBatch({ selectBrands: selectBrands }).then(response => {
    ...
    handleUpdateSort() {
      this.$confirm('是否要修改排序?', '提示', {
        ...
        sms.updateBrandRecommendSort(this.sortDialogData).then(response => {
    ...
    getList() {
      ...
      sms.getBrandRecommendList(this.listQuery).then(res => {
    ...
    updateRecommendStatusStatus(ids, status, exc_keys) {
      this.$confirm('是否要修改推荐状态?', '提示', {
        ...
        sms.updateBrandRecommendStatus({
          ids: ids,
          exc_keys: exc_keys,
          status: status
        }).then(response => {
    ...
    deleteBrand(ids) {
      this.$confirm('是否要删除该推荐?', '提示', {
        ...
        sms.deleteHomeBrand({ pk: ids.join(",") }).then(response => {
          this.getList();
          this.$message({
            type: 'success',
            message: '删成功!'
    ...
    getDialogList() {
      pms.getBrandList(this.dialogData.listQuery).then(res => {
