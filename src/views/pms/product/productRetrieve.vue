      ...
      <div style="margin-top: 15px">
        <el-form :inline="true" :model="listQuery" label-width="140px" size="small">
          ...
          <el-form-item label="商品分类：">
            <el-cascader :options="productCateOptions" :props="{value:'id', label:'name',children:'childList'}" clearable v-model="selectProductCateValue"></el-cascader>
          </el-form-item>
    ...
    <div class="table-container">
      <el-table :data="list" @selection-change="handleSelectionChange" border ref="productTable" style="width: 100%" v-loading="listLoading">
        ...
        <el-table-column align="center" label="操作" width="160">
          <template slot-scope="scope">
            <p>
              <el-button @click="saveDrStatus(scope.row.id)" size="mini" type="primary">恢复</el-button>
            </p>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      pms.getProductListByDeleted(this.listQuery).then(res => {
    ...
    handleUpdateProduct(index, row) {
      ...
      utility.addtagview(this, "编辑商品", "productUpdate", row.id);
    },
    ...
    saveDrStatus(id) {
      ...
      this.$confirm('是否要进行恢复操作?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        pms.saveDrStatus({ id: id }).then(res => {
          this.getList();
          this.$message({
            message: '修改成功',
            type: 'success',
