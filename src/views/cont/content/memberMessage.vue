  ...
  <div class="pagecontent">
    ...
    <cwindow :title="wintitle" v-model="winvisible">
      <div class="page-controlstyle">
        <cinput :required="true" title="接收人id" v-model="mm.member_id"></cinput>
        <cinput :required="true" title="发送人id" v-model="mm.source_id"></cinput>
        <cinput :required="true" title="标题" v-model="mm.title"></cinput>
        <cinput :required="true" title="内容" v-model="mm.content"></cinput>
        <cdate :required="true" title="发送时间" v-model="mm.send_time"></cdate>
        <cinput title="跳转路径" v-model="mm.redirect_url"></cinput>
      </div>
...
export default {
  ...
  methods: {
    ...
    showEdit(row) {
      let that = this;
      u.openeditmodel(this, cont.getMemberMessage, row).then(res => {
        console.log(res)
        that.timeValue = [res.start_time, res.end_time]
      });;
    },
    doDelete() {
      u.deleteoperate(this, cont.deleteMemberMessage);
    },
    doExport() {
    ...
    doSave() {
      u.doSave(this, cont.saveMemberMessage);
    },
