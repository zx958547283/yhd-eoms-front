  ...
  <div class="pagecontent">
    ...
    <cwindow :title="wintitle" v-model="winvisible">
      <div class="page-controlstyle">
        <cinput :required="true" title="分享人id" v-model="mm.member_id"></cinput>
        <cinput title="分享对象id" v-model="mm.target_member_id"></cinput>
        <cinput title="分享内容id" v-model="mm.union_id"></cinput>
        <cdate :required="true" title="分享时间" v-model="mm.share_time"></cdate>
      </div>
...
export default {
  ...
  methods: {
    ...
    showEdit(row) {
      let that = this;
      u.openeditmodel(this, cont.getShare, row).then(res => {
        console.log(res)
        that.timeValue = [res.start_time, res.end_time]
      });;
    },
    doDelete() {
      u.deleteoperate(this, cont.deleteShare);
    },
    doExport() {
