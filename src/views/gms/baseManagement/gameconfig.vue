  ...
  <div class="app-container">
    ...
    <cwindow :title="wintitle" v-model="winvisible">
      <div class="page-controlstyle">
        <cinput :required="true" title="配置key" v-model="mm.config_key"></cinput>
        <cinput :required="true" title="配置value" v-model="mm.config_value"></cinput>
        <cinput :required="true" title="配置变量值" v-model="mm.config_variable"></cinput>
        <cinput :length="2" title="配置说明" v-model="mm.config_desc"></cinput>
        <cselect :required="true" querypk="9f977068-75a7-439e-ba56-60e1807fd333" title="所属游戏" v-model="mm.game_tag"></cselect>
      </div>
...
export default {
  ...
  methods: {
    ...
    doQuery(querylist) {
      ...
      if (querylist === undefined) {
        if (u.isnullorwhitespace(this.queryvalue)) {
          ...
          this.querylist = [];
          u.addwheres(this, "and", "(", "gms_game_config.config_key", "like", this.queryvalue, "");
          u.addwheres(this, "or", "", "gms_game_config.config_desc", "like", this.queryvalue, ")");
        }
      } else {
    ...
    showEdit(row) {
      let that = this;
      u.openeditmodel(this, gms.getGameConfig, row);
    },
    doDelete() {
      u.deleteoperate(this, gms.deleteGameConfig);
    },
    doExport() {
    ...
    doSave() {
      u.doSave(this, gms.saveGameConfig);
    }
  }
}
</script>
