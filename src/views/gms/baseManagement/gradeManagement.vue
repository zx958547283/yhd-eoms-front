  ...
  <div class="app-container">
    ...
    <cwindow :title="wintitle" v-model="winvisible">
      <div class="page-controlstyle">
        <cinput :required="true" title="段位名称" v-model="mm.grade_name"></cinput>
        <cnumber :required="true" title="升级连胜次数" v-model="mm.grade_need_score"></cnumber>
        <cnumber :required="true" title="扣除粮票数" v-model="mm.grade_food_cnt"></cnumber>
        <csuploadWithPath :dataObj="updatePath" :length="2" title="段位icon" v-model="mm.grade_icon"></csuploadWithPath>
      </div>
...
export default {
  data() {
    return {
      ...
      updatePath: {
        path: process.env.GRADE_ICON_PATH
      }
    }
  },
  mounted() {
  ...
  methods: {
    ...
    doQuery(querylist) {
      if (querylist === undefined) {
        if (u.isnullorwhitespace(this.queryvalue)) {
          ...
          this.querylist = [];
          u.addwheres(this, "and", "(", "gms_grade.grade_name", "like", this.queryvalue, "");
          u.addwheres(this, "or", "", "gms_grade.grade_need_score", "like", this.queryvalue, ")");
        }
      } else {
    ...
    showEdit(row) {
      let that = this;
      u.openeditmodel(this, gms.getGrade, row);
    },
    doDelete() {
      u.deleteoperate(this, gms.deleteGrade);
    },
    doExport() {
