  ...
  <div class="app-container">
    ...
    <cwindow :title="wintitle" v-model="winvisible">
      <div class="page-controlstyle">
        <cinput :required="true" title="名称" v-model="mm.bgm_name"></cinput>
        <cselect :required="true" pk="openFlg" title="开启状态" v-model="mm.status"></cselect>
        <cnumber :required="true" title="排序" v-model="mm.sort"></cnumber>
        <csuploadWithPath :dataObj="updatePath" :length="2" fileType="other" title="等级icon" v-model="mm.bgm_path"></csuploadWithPath>
      </div>
...
export default {
  ...
  methods: {
    ...
    doQuery(querylist) {
      if (querylist === undefined) {
        if (u.isnullorwhitespace(this.queryvalue)) {
          ...
          this.querylist = [];
          u.addwheres(this, "and", "(", "gms_level.bgm_name", "like", this.queryvalue, "");
          u.addwheres(this, "or", "", "gms_level.id", "like", this.queryvalue, ")");
        }
      } else {
    ...
    showEdit(row) {
      let that = this;
      u.openeditmodel(this, gms.getBgmSettings, row);
    },
    doDelete() {
      u.deleteoperate(this, gms.deleteBgmSettings);
    },
    doExport() {
    ...
    doSave() {
      if (this.mm.bgm_path === '' || this.mm.bgm_path === null || this.mm.bgm_path === undefined) {
        this.$message({
          message: '请上传文件',
          type: 'warning',
          duration: 1000
      ...
      u.doSave(this, gms.saveBgmSettings);
    }
  }
}
</script>
