  ...
  <div class="app-container">
    ...
    <cwindow :title="wintitle" v-model="winvisible">
      <div class="page-controlstyle">
        <cinput :disabled="true" :required="true" title="段位名称" v-model="mm.grade_name"></cinput>
        <cnumber :required="true" title="答题正确率" v-model="mm.status"></cnumber>
      </div>
...
export default {
  ...
  methods: {
    ...
    doSave() {
      console.log(this.mm)
      if (this.mm.status === 'undefined' || null === this.mm.status || '' === this.mm.status) {
        this.$message.error('机器人胜率必须输入！');
        return;
      }
