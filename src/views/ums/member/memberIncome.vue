...
export default {
  ...
  methods: {
    queryIncome(row) {
      ...
      u.addtagview(this, "会员收益详情", "memberIncomeDetail", row.pk);
    },
    doQuery(treeNode) {
      this.querylist = [];
      if (treeNode) {
    ...
    btnquery: function (data) {
      ...
      if (u.isnullorwhitespace(data)) {
        u.addwherelike(this, "ums_member.nickname", this.queryvalue);
      } else {
        // u.addwheres(this, "and", "(", "Category.fpk", "=", data.pk, "");
        // u.addwheres(this, "or", "", "Category.pk_Category", "=", data.pk, ")");
      }
      this.dataLoad(1);
    }
