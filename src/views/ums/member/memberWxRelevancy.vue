  ...
  <div class="pagecontent">
    ...
    <cwindow :title="wintitle" v-model="winvisible">
      <div class="page-controlstyle">
        ...
        <cnumber :required="true" title="用户来源（0：前贝课堂公众号 1：前贝学堂小程序 2：乐龄唱响）" v-model="mm.source_type"></cnumber>
        <cinput :required="true" title="微信id" v-model="mm.wx_openId"></cinput>
      </div>
...
export default {
  ...
  methods: {
    ...
    showEdit(row) {
      let that = this;
      u.openeditmodel(this, ums.getMemberWxRelevancy, row).then(res => {
        console.log(res)
        that.timeValue = [res.start_time, res.end_time]
      });;
    },
    doDelete() {
      u.deleteoperate(this, ums.deleteMemberWxRelevancy);
    },
    doExport() {
    ...
    doSave() {
      u.doSave(this, ums.saveMemberWxRelevancy);
    },
