    ...
    <cwindow :title="wintitle" v-model="winvisible">
      <div class="page-controlstyle">
                                                              <cinput :required="true" title="会员id" v-model="mm.member_id"></cinput>
                                                                              <cinput :required="true" title="任务id" v-model="mm.job_id"></cinput>
                                                                              <cdate :required="true" title="任务执行时间" v-model="mm.execute_time"></cdate>
                                                                              <cnumber :required="true" title="获得积分" v-model="mm.score"></cnumber>
                                                                                                                              </div>
...
export default {
  ...
  methods: {
    ...
    showEdit(row) {
      let that = this;
      u.openeditmodel(this, job.getMemberJob, row).then(res => {
        console.log(res)
        that.timeValue = [res.start_time, res.end_time]
      });;
    },
    doDelete() {
      u.deleteoperate(this, job.deleteMemberJob);
    },
    doExport() {
    ...
    doSave() {
            u.doSave(this, job.saveMemberJob);
    },
